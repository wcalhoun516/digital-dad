...
**Chronological — oldest first, newest at the bottom.** The agent appends one entry per run at
the **very end of this file**. It tallies the `Category:` of the last 7 entries (the bottom 7)
to decide the least-recently-worked category for cold-path selection.

> **Never delete an existing entry.** If a rebase or merge conflicts here, the resolution is
> always **keep both** and order by date — two runs appending on the same day is not a choice
> between them. Five entries (PRs #92–#96) were silently lost on 2026-09-19 exactly this way,
> which corrupted the category tally that drives cold-path selection.
